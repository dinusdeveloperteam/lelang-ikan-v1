		...
		if ($this->form_validation->run() == false) {
			...
			if ($query->num_rows() > 0) {
				...
				if ($data->role == 1) {
					...
					// Jika Role Bukan Samadengan 1 dan input username dan password sudah benar maka akan ditampilkan Session dibawah ini
					$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">' . 'Login Gagal, pastikan username dan password benar!.' . '</div>');
